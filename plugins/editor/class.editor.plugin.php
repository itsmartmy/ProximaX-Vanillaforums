...
class EditorPlugin extends Gdn_Plugin {
    ...
    public function base_render_before($sender) {
        ...
        $c->addJsFile('checkDownload.js', 'plugins/editor');
    ...
    public function postController_editorUpload_create($sender, $args = []) {
        ...
        if ($tmpFilePath && $canUpload) {
            ...
            // ========================== Added code to support Proximax ========================
            $file_path = $media['Path'];
            // Your HTTP path to vanilla forum upload directory (For example: http://localhost:8888/vanilla-core/uploads/)
            $upload_url = '<YOUR HTTP PATH TO VANILLA FORUM UPLOAD DIRECTORY>'.$media['Path'];
            $path_parts = pathinfo($media['Name']);
            $attachment_name = $path_parts['filename'];
            $file_ext = '.'.$path_parts['extension'];
            $senderPrivateKey = '<YOUR SENDER PRIVATE KEY>';
            $receiverPublicKey = '<YOUR RECEIVER PUBLIC KEY>';
            $keywords = 'Vanilla Forum,attachment,comment';
            $MetadataKey = "Media_ID";
            $MetadataValue = $mediaID;
            $apiKey = '<YOUR API KEY GENERATED BY ITSART>';
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL,"http://prx-devel.itsmart.my:8080/ProximaxAPI/webresources/uploadAttachment");
            curl_setopt($ch,CURLOPT_POST,true);
            curl_setopt($ch, CURLOPT_POSTFIELDS,"uploadUrl=".$upload_url."&attachmentName=".$attachment_name."&fileExtension=".$file_ext."&senderPrivateKey=".$senderPrivateKey."&receiverPublicKey=".$receiverPublicKey."&keywords=".$keywords."&MetadataKey=".$MetadataKey."&MetadataValue=".$MetadataValue."&apiKey=".$apiKey);
            ...
            $media2 = [
                'MediaID' => $mediaID,
                'Path' => 'https://testnet.gateway.proximax.io/xpxfs/'.$server_output,
            ];
            $model->save($media2);
            //rmdir('../../uploads/editor/'.$tmpFilePath);
            $data = explode("/",$file_path);
            $data = array_values($data);
            $dirname = './uploads/editor/'.$data[1];
            ...
            // ========================== End of code to support Proximax ========================
